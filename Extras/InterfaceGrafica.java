...
public class InterfaceGrafica {
    ...
    private static void criarProjeto(Professor professor) {
        ...
        if(!dataValida(dataInicio)){
            JOptionPane.showMessageDialog(null, "Erro: Data de ínicio inválida ou no passado!", 
        ...
        try{
            ...
            java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy");
                java.time.LocalDate inicio =
                  java.time.LocalDate.parse(dataInicio, fmt);
                     java.time.LocalDate fim = java.time.LocalDate.parse(prazo, fmt);

                     if(!fim.isAfter(inicio)){
                        JOptionPane.showMessageDialog(null, "Erro: O prazo deve ser posterior à data de início",
                            "Erro", JOptionPane.ERROR_MESSAGE);
                            return;
                     }
        }catch(Exception e){
        ...
        if (escolha == 0) {
            ...
            if (selected != null) {
                ...
                if(!dataValida(dataIni)){
                    JOptionPane.showMessageDialog(null, "Erro: Data início ou no passado! ",
                        "Erro", JOptionPane.ERROR_MESSAGE);
                        return;
                }
                if(!dataValida(prazo)){
